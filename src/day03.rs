...
fn get_instructions(string: &str) -> Vec<Instruction> {
    let raw_string = string; // super::utils::get_string_from_stdio();
    return raw_string
        .trim()
        .split(",")
...
fn get_state() -> Vec<Vec<u32>> {
    return vec![vec![0; (OFFSET * 2) as usize]; (OFFSET * 2) as usize];
}
...
fn apply_instructions(
    ...
    for instruction in instructions {
        ...
        // println!(
        //     "> {:?} --> {:?}: {:?} {:?}",
        //     coordinate, next_coordinate, dx, dy
        // );

        // Traversal
        // TODO: Feels like something is wrong here
        if dx.abs() > 0 {
            let range;
            if dx > 0 {
                range = 1..(dx + 1);
            ...
            for ddx in range {
                let current_value = new_state[coordinate.y as usize][(coordinate.x + ddx) as usize];
                let new_value;
                if set_absolute {
                    new_value = 1;
                } else {
                ...
                new_state[coordinate.y as usize][(coordinate.x + ddx) as usize] = new_value;
            }
        }
        if dy.abs() > 0 {
            let range;
            ...
            for ddy in range {
                let current_value = new_state[(coordinate.y + ddy) as usize][coordinate.x as usize];
                let new_value;
                if set_absolute {
                    new_value = 1;
                ...
                new_state[(coordinate.y + ddy) as usize][coordinate.x as usize] = new_value;
            }
        }
...
fn get_answer(state: &Vec<Vec<u32>>) -> u32 {
    let mut answer = std::u32::MAX;

    for y in 0..state.len() {
        for x in 0..state[y].len() {
            if state[y][x] > 1 && y as i32 != OFFSET && x as i32 != OFFSET {
                let value = (x as i32 - OFFSET).abs() as u32 + (y as i32 - OFFSET).abs() as u32;
                // println!("> {:?} {:?}; {}", x, y, value);
                answer = std::cmp::min(value, answer);
            }
        }
    }
...
fn print_state(state: &Vec<Vec<u32>>) {
    for y in 0..state.len() {
        for x in 0..state[y].len() {
            print!("{},", state[y][x]);
        }
...
pub fn main() {
    ...
    // print_state(&state);
    state = apply_instructions(&state, inst_1, true);
    // print_state(&state);
    state = apply_instructions(&state, inst_2, false);
    print_state(&state);

    // Find a state[y][x] that is closest to state[OFFSET][OFFSET]
